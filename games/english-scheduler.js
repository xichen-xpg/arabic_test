...
(function (root) {
  ...
  class Scheduler {
    ...
    plan(date = this.clock(), create = date === this.clock()) {
      ...
      const review = due.slice(0, 30).map(([id]) => id);
      const fresh = this.bank.filter(word => !state.records[word.id]).slice(0, Math.min(20, 30 - review.length)).map(word => word.id);
      const plan = { date, review, fresh, completed: [], failed: [], retry: [], drafts: {}, deferred: Math.max(0, due.length - review.length) };
  ...
  const api = { Scheduler, today, normalize, addDays, STORAGE_KEY, INTERVALS };
